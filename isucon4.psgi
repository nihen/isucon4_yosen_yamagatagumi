...
sub app {
    ...
    if ( $method eq 'GET' ) {
        if ( $path_info eq '/mypage' ) {
            ...
            if ( $user ) {
                return ['200', $header, [
                    \q{<!DOCTYPE html><html><head><meta charset="UTF-8"><title>isucon4</title></head><body>},
                    \sprintf('<div id="last-logined-at">%s</div>', $user->{last_login1} ? strftime('%Y-%m-%d %H:%M:%S', localtime($user->{last_login1})) : ''),
                    \sprintf('<div id="last-logined-ip">%s</div>', $user->{last_ip1} ? $user->{last_ip1} : ''),
                    \sprintf('<div id="login">%s</div>', $user->{login}),
                    \q{<script>document.write('<script src="/javascripts/mypage.html.js"><\\/script>')</script></body></html>},
                ]];
            }
